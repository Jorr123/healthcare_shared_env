...
ENV_REQUEST = RecorderEnvRequest(
    target="scalpel",
    distractors=("scissor", "love_retractor", "kelly", "scalpel_type2"),
    environment=None,
)
...
KELLY_USD = asset_path("kelly_root_at_center.usd")
...
# Scissor: keep old working grasp logic.
SCISSOR_GRASP_ABOVE_TABLE = 0.005
SCISSOR_BODY_OFFSET_X = 0.0
SCISSOR_BODY_OFFSET_Y = 0.0
SCISSOR_BODY_OFFSET_Z = -0.008
...
SCALPEL_FLAT_LONG_AXIS = "X"
SCALPEL_EDGE_LONG_AXIS = "Y"
...
SCISSOR_RANDOM_X_RANGE = (0.34, 0.50)
SCISSOR_RANDOM_Y_RANGE = (0.04, 0.28)
...
# if target=scalpel -> distractor=scissor
# if target=scissor -> distractor=scalpel
...
SCISSOR_SLOT_OFFSET = [0.0,  0.055, 0.025]
...
parser.add_argument("--task_text",     type=str,
    default="Pick up the scalpel and place it in the right tray slot")
...
parser.add_argument("--layout_tuner", action="store_true",
                    help="Open shared robot/table/hospital layout GUI; save transforms when the window closes.")
parser.add_argument("--workspace_tuner", action="store_true",
                    help="Translate Robot and Table together and save the matching recorder workspace offset.")
parser.add_argument("--layout_file", type=str,
                    default=str(os.path.join(os.path.dirname(__file__), "shared_layout.json")))
...
STATE_KEYS = [
    ...
    "ee_pos_bx","ee_pos_by","ee_pos_bz",
    "ee_quat_bw","ee_quat_bx","ee_quat_by","ee_quat_bz",
    "scissor_pos_bx","scissor_pos_by","scissor_pos_bz",
    "scissor_quat_bw","scissor_quat_bx","scissor_quat_by","scissor_quat_bz",
...
def get_scissor_pos_w(env):  return env.scene["object"].data.root_pos_w[0].clone()
def get_scissor_quat_w(env): return env.scene["object"].data.root_quat_w[0].clone()
...
def make_state(env, active_target_w, phase_id):
    ...
    st = torch.cat([
        ...
        to_base_pos(env,  get_scissor_pos_w(env)),
        to_base_quat(env, get_scissor_quat_w(env)),
...
def step_dynamic(env, recorder, name, target_w, quat_w, grip,
    ...
    for i in range(max_steps):
        ...
        obj = get_scalpel_pos_w(env) if phase_id==0 else get_scissor_pos_w(env)
...
def step_hold_const_grip(env, recorder, name, target_w, quat_w,
    ...
    Use this for scissor close: no delayed ramp, so the fingers clamp before lift.
    """
    max_z = -999.0
    for i in range(steps):
        st = make_state(env, tray_slot_w, phase_id)
...
class EpisodeRecorder:
    ...
    def save_episode(self, ep_idx, success, meta=None):
        ...
        with h5py.File(path, "w") as f:
            ...
            f.attrs["pick_order"] = "scalpel_only"
            f.attrs["task_mode"] = "scalpel_only"
            f.attrs["scalpel_slot"] = "right"
            f.attrs["scissor_slot"] = "left"
...
def _sample_extra_obj_params(rng, obj_name):
    ...
    if obj_name == "love_retractor":
        # Love uses x/y root, same pick table region.
...
def ensure_two_distractors(spawn_params, target_object, rng):
    """Ensure spawn_params contains scissor, scalpel, love_retractor.
...
def debug_print_object_positions(env, tag=""):
    for key in ["object", "scalpel", "scissor", "love_retractor", "kelly"]:
...
def force_extra_distractors(env, spawn_params, target_object):
    ...
    # love_retractor as extra scene key
    ...
    # kelly as extra scene key
    if target_object != "kelly" and "kelly" in spawn_params and "kelly" in env.scene.keys():
        p = spawn_params["kelly"]
        z = float(globals().get("KELLY_SPAWN_ROOT_Z", 0.0010))
        _write_rigid_pose(env, "kelly", [float(p["x"]), float(p["y"]), z], float(p["yaw_deg"]))
        print(f"[SPAWN EXTRA KELLY] root=({p['x']:.4f},{p['y']:.4f},{z:.4f}) yaw={p['yaw_deg']:.1f}")
...
def stage_suffix_from_name(stage_name):
    st = str(stage_name)
    for pfx in ("SCISSOR_", "SCALPEL_", "LOVE_", "LOVE_RETRACTOR_", "KELLY_"):
...
def save_realcompat_segment(recorder, ep_idx, segment, out_dir, success, object_name, meta=None):
    ...
    with h5py.File(path, "w") as f:
        ...
        f.attrs["distractor_policy_note"] = "target=scalpel; expected distractors: scissor and optional love_retractor when configured"
        f.attrs["state_frame"] = "robot_base"
        f.attrs["action_frame"] = "robot_base"
        f.attrs["action_type"] = "absolute_pose_quat_gripper_8d_baseframe"
        phase3_hide_all_debug_visuals(omni.usd.get_context().get_stage())
        ...
        dbg.create_dataset("scissor_pose_b", data=states_legacy[:, 16:23])
        dbg.create_dataset("scalpel_pose_b", data=states_legacy[:, 23:30])
        if object_name_l == "scissor":
...
def sample_tray_center(rng, device, scissor_pos_w, scalpel_center_w):
...
def force_scissor_pose(env, spawn_params):
    ...
    q = yaw_quat_wxyz(torch.tensor(yaw_rad, device=env.device), env.device)
    root_pose = torch.zeros((env.num_envs, 7), device=env.device)
    root_pose[:, 0:3] = torch.tensor(
        [float(spawn_params["x"]), float(spawn_params["y"]), SCISSOR_SPAWN_ROOT_Z],
        device=env.device
    ).reshape(1,3)
    root_pose[:, 3:7] = q.reshape(1,4)
    root_vel = torch.zeros((env.num_envs, 6), device=env.device)
    ...
    obj.write_root_velocity_to_sim(root_vel)
    obj.update(dt=env.physics_dt)
    print(f"[SPAWN SCISSOR] root=({spawn_params['x']:.4f},{spawn_params['y']:.4f},{SCISSOR_SPAWN_ROOT_Z:.4f}) yaw={spawn_params['yaw_deg']:.1f}")
...
def force_scalpel_pose(env, pose_mode, spawn_params):
    ...
    print(
        f"[SPAWN SCALPEL OLDLOGIC] mode={pose_mode} "
        f"center=({desired_center[0]:.4f},{desired_center[1]:.4f},{desired_center[2]:.4f}) "
        f"yaw={yaw_deg:.1f} root={[round(float(v),4) for v in root_pos.tolist()]}"
    )
...
def compute_scalpel_grasp_pose(env, obj_raw, obj_quat, pose_mode):
    # OLD VERIFIED LOGIC:
    # target center is NOT root, NOT mesh centroid, NOT fixed world point.
    # center_w = root + quat_apply(root_quat, SCALPEL_LOCAL_CENTER)
    ...
    print("")
    print(f"[SCALPEL OLDLOGIC] origin=({float(obj_raw[0]):.4f},{float(obj_raw[1]):.4f},{float(obj_raw[2]):.4f}) mode={pose_mode}")
...
# SCISSOR branch is kept as before.
...
def pick_and_place_object(
    ...
    if obj_name == "SCALPEL":
        ...
        # Axis markers are hidden when SHOW_MARKERS=False.
        ...
        # SCISSOR: same working yaw-only logic as old script.
        ...
        print(f"\n[SCISSOR] origin=({float(obj_raw[0]):.4f},{float(obj_raw[1]):.4f},{float(obj_raw[2]):.4f})")
    ...
    hold_steps=20 if obj_name=="SCISSOR" else 30
    ...
    if obj_name == "SCISSOR":
        # SCISSOR: close must be immediate and stationary.
        # Do not use step_smooth_grip here, because its first 25% stays open
        # and the scissor can slip before the fingers fully clamp.
        ...
        scissor_micro_lift = close_grasp + torch.tensor([0.0,0.0,0.070], device=env.device)
        mz=max(mz,step_dynamic(env,recorder,pfx+"MICRO_LIFT",scissor_micro_lift,grasp_quat,-1.0,
    ...
    lift_thresh=0.006 if obj_name=="SCISSOR" else 0.008
...
PHASE3_TARGET_OBJECT = "scalpel"
PHASE3_TARGET_OBJECT_TYPE_ID = 0
...
def phase3_ensure_scalpel_type2_scene_cfg(env_cfg):
    """
    Ensure scalpel recorder has scalpel_type2 as real fifth object.
    """
    try:
        if hasattr(env_cfg.scene, "scalpel_type2"):
            return
        ...
        env_cfg.scene.scalpel_type2 = RigidObjectCfg(
            ...
            spawn=UsdFileCfg(
                semantic_tags=[("class", "scalpel_type2")],
                usd_path=SCALPEL_TYPE2_USD_PATH,
                scale=(0.00475421, 0.00475421, 0.00475421),
            ...
            init_state=RigidObjectCfg.InitialStateCfg(pos=(0.62, 0.30, 0.006), rot=(1.0, 0.0, 0.0, 0.0)),
        )
        print("[SCENE CFG] added scalpel_type2 fifth object")
    except Exception as e:
        print("[SCENE CFG ERROR] add scalpel_type2 failed:", repr(e))

def phase3_ensure_all_5_objects_in_spawn(spawn_params, target_object, rng):
    """
    Final runtime fix for grid spawn path:
      sample_episode_spawn_grid(...)
      ensure_two_distractors(...)
    may still return only 4 objects. This guarantees:
      scalpel, scissor, love_retractor, kelly, scalpel_type2
    """
    import math
    ...
    def sample_one(name):
        ...
        raise RuntimeError(f"unknown object for spawn sample: {name}")
...
# =============================================================================
# PHASE3 NATIVE DISTRACTOR FORCE FUNCTIONS - SAFE SCENE KEY CHECK
# =============================================================================
...
def force_scalpel_type2_pose(env, spawn_params):
    ...
    phase3_write_rigid_root_pose(env, "scalpel_type2", [x, y, z], q, "SCALPEL_TYPE2")
...
def _layout_local_pose(stage, path):
    prim = stage.GetPrimAtPath(path)
    if not prim.IsValid():
        return None
    # Save the authored transform relative to /World/envs/env_0. Reapplying a
    # world transform as an init/local transform double-counts the env parent.
    matrix = UsdGeom.Xformable(prim).GetLocalTransformation()
    pos = matrix.ExtractTranslation()
    quat = matrix.ExtractRotationQuat()
    imag = quat.GetImaginary()
    scale = Gf.Transform(matrix).GetScale()
    quat_values = [float(quat.GetReal()), float(imag[0]), float(imag[1]), float(imag[2])]
    quat_norm = sum(v * v for v in quat_values) ** 0.5
    quat_values = [v / quat_norm for v in quat_values] if quat_norm > 1.0e-8 else [1.0, 0.0, 0.0, 0.0]
    return {
        "path": path,
...
def _find_layout_path(stage, exact_path, token):
    if stage.GetPrimAtPath(exact_path).IsValid():
        return exact_path
    matches = [str(p.GetPath()) for p in stage.Traverse()
               if token.lower() in p.GetName().lower()]
    return min(matches, key=lambda p: (p.count("/"), len(p))) if matches else None


def run_shared_layout_tuner(env, stage, save_path):
    robot_path = _find_layout_path(stage, "/World/envs/env_0/Robot", "robot")
    table_path = _find_layout_path(stage, "/World/envs/env_0/Table", "table")
    room_path = _find_layout_path(stage, "/World/envs/env_0/HospitalRoom", "hospitalroom")
    print("\n[LAYOUT TUNER READY]")
    print("  Pause timeline if it is playing.")
    print("  Keep Robot and Table unchanged; they are the validated recorder workspace.")
    print("  Move/Rotate/Scale only this top-level environment prim:")
    print("  Use the Scale gizmo on HospitalRoom; keep Robot/Table scale unchanged.")
    print("  robot   =", robot_path)
    print("  table   =", table_path)
    print("  hospital=", room_path)
    print("  Layout autosaves while open. Press Ctrl+C in PowerShell for final save and exit:", save_path)
    ...
    def save_room(label):
        payload = {
            ...
            "table": None,
            "environment": {
                "hospital_room": _layout_local_pose(stage, room_path)
            } if room_path else {},
            "active_environment": ["hospital_room"],
        }
    ...
    try:
        while simulation_app.is_running():
            ...
            frame += 1
            if frame % 120 == 0:
                save_room("[LAYOUT AUTOSAVED]")
    except KeyboardInterrupt:
        print("[LAYOUT TUNER] Ctrl+C received")
    save_room("[LAYOUT SAVED]")


def run_workspace_layout_tuner(env, stage, save_path):
    """Capture one common translation for Robot, Table, and recorder coordinates."""
    robot_path = _find_layout_path(stage, "/World/envs/env_0/Robot", "robot")
    table_path = _find_layout_path(stage, "/World/envs/env_0/Table", "table")
    room_path = _find_layout_path(stage, "/World/envs/env_0/HospitalRoom", "hospitalroom")
    robot_start = _layout_local_pose(stage, robot_path)
    table_start = _layout_local_pose(stage, table_path)
    with open(save_path, "r", encoding="utf-8") as stream:
        old_payload = json.load(stream)
    old_offset = old_payload.get("workspace_offset", [0.0, 0.0, 0.0])

    print("\n[WORKSPACE TUNER READY]")
    print("  Pause timeline. Translate Robot and Table by EXACTLY the same XYZ amount.")
    print("  Do NOT rotate or scale Robot/Table. HospitalRoom is already saved.")
    print("  robot =", robot_path)
    print("  table =", table_path)
    print("  Autosave occurs only when their translation deltas match within 5 mm.")
    print("  Press Ctrl+C in PowerShell for final validation/save:", save_path)

    def try_save(label):
        robot = _layout_local_pose(stage, robot_path)
        table = _layout_local_pose(stage, table_path)
        robot_delta = [robot["pos"][i] - robot_start["pos"][i] for i in range(3)]
        table_delta = [table["pos"][i] - table_start["pos"][i] for i in range(3)]
        mismatch = max(abs(robot_delta[i] - table_delta[i]) for i in range(3))
        if mismatch > 0.005:
            print(f"[WORKSPACE NOT SAVED] Robot/Table delta mismatch={mismatch:.4f} m; make XYZ moves identical.")
            return False
        payload = {
            "robot": robot,
            "table": table,
            "workspace_offset": [old_offset[i] + table_delta[i] for i in range(3)],
            "environment": old_payload.get("environment", {
                "hospital_room": _layout_local_pose(stage, room_path)
            } if room_path else {}),
            "active_environment": old_payload.get("active_environment", ["hospital_room"]),
        }
        with open(save_path, "w", encoding="utf-8") as stream:
            json.dump(payload, stream, indent=2)
        print(label, save_path, "offset=", payload["workspace_offset"])
        return True
    ...
    try:
        while simulation_app.is_running():
            ...
            frame += 1
            if frame % 120 == 0:
                try_save("[WORKSPACE AUTOSAVED]")
    except KeyboardInterrupt:
        print("[WORKSPACE TUNER] Ctrl+C received")
    try_save("[WORKSPACE SAVED]")


def main():
    global ALL_PHASE3_OBJECTS, PHASE3_ALL_OBJECTS, ENV_REQUEST
    apply_workspace_globals(globals())
    if args_cli.layout_tuner or args_cli.workspace_tuner:
        ENV_REQUEST = RecorderEnvRequest(
            target="scalpel", distractors=(),
            environment=("hospital_room",), use_canonical_target_only=False)
    ALL_PHASE3_OBJECTS = ENV_REQUEST.instruments
    PHASE3_ALL_OBJECTS = ENV_REQUEST.instruments
    print("[CONFIG] SCALPEL ONLY | direct random spawn | same schema for shared multitask training")
    ...
    print(f"  KELLY_SCALE={KELLY_SCALE} KELLY_SPAWN_ROOT_Z={KELLY_SPAWN_ROOT_Z}")
    ...
    print("  SCISSOR_CLOSE=hard constant -1.0, then HOLD_AFTER_CLOSE before micro-lift")

    env_cfg=parse_env_cfg(args_cli.task,device=args_cli.device,num_envs=args_cli.num_envs)
    phase3_ensure_all_object_scene_cfg(env_cfg)

    phase3_ensure_scalpel_type2_scene_cfg(env_cfg)
    try:
    ...
    env_cfg.scene.object.spawn=UsdFileCfg(
        usd_path=SCALPEL_USD,scale=(1.0,1.0,1.0),
    ...
    if hasattr(env_cfg,"events") and hasattr(env_cfg.events,"reset_object_position"):
        env_cfg.events.reset_object_position.params["asset_cfg"].body_names=["Surgical_Knife_fbx"]
    ...
    # Extra distractor #2: love_retractor
    ...
    # Extra distractor: kelly
    env_cfg.scene.kelly=RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/Kelly",
        spawn=_UF(
            semantic_tags=[("class", "kelly")],usd_path=KELLY_USD_PATH,scale=KELLY_SCALE,
                  rigid_props=RigidBodyPropertiesCfg(
                      disable_gravity=False,
                      linear_damping=0.8,
                      angular_damping=4.0,
                  ...
                  collision_props=CollisionPropertiesCfg(contact_offset=0.003,rest_offset=0.0),
                  mass_props=MassPropertiesCfg(mass=0.10)),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.45,0.30,0.002),rot=(1.0,0.0,0.0,0.0)),
    )


    # Extra distractor: scissor
    ...
    if args_cli.layout_tuner or args_cli.workspace_tuner:
        if args_cli.workspace_tuner:
            run_workspace_layout_tuner(env, stage, args_cli.layout_file)
        else:
            run_shared_layout_tuner(env, stage, args_cli.layout_file)
        env.close()
    ...
    while saved_count < target_success:
        ...
        spawn_params=sample_episode_spawn_grid(rng, attempt, "scalpel")
        spawn_params=ensure_two_distractors(spawn_params, "scalpel", rng)
        spawn_params=phase3_ensure_all_5_objects_in_spawn(spawn_params, "scalpel", rng)
        ...
        # DISABLED: old duplicate distractor logic
        # force_extra_distractors(env, spawn_params, "scalpel")
        ...
        scissor_raw=get_scissor_pos_w(env)
        scalpel_center=get_scalpel_center_w(env)

        dist_obj=torch.linalg.norm((scissor_raw-scalpel_center)[:2]).item()
        if dist_obj < SPAWN_MIN_OBJ_OBJ:
            print(f"[SPAWN FAIL] too close after pose write ({dist_obj:.3f}m)"); recorder._reset(); continue

        tray_center=sample_tray_center(rng, env.device, scissor_raw, scalpel_center)
        ...
        scissor_slot_w=tray_center+torch.tensor(SCISSOR_SLOT_OFFSET,device=env.device)
        scalpel_slot_w=tray_center+torch.tensor(SCALPEL_SLOT_OFFSET,device=env.device)
        print(f"  scissor slot: {[round(x,4) for x in scissor_slot_w.tolist()]}")
        print(f"  scalpel slot: {[round(x,4) for x in scalpel_slot_w.tolist()]}")

        # ?? SINGLE-OBJECT PHASE: SCALPEL ONLY ?????????????????????????????
        print("\n[PHASE] Scalpel -> right slot | phase_id=0")
        sp_ok,sp_mz,sp_cz,sp_fp=pick_and_place_object(
            env,recorder,stage,"SCALPEL",0,
            SCALPEL_GRASP_Z_ABOVE_TABLE,
            SCALPEL_BODY_OFFSET_X,SCALPEL_BODY_OFFSET_Y,SCALPEL_BODY_OFFSET_Z,
            get_scalpel_pos_w,get_scalpel_quat_w,scalpel_slot_w,base_grip_quat,
            scalpel_pose_mode=scalpel_pose_mode)

        if not sp_ok:
            print("[PHASE FAIL] scalpel micro-lift"); recorder._reset(); continue
        if not quality_ok(recorder,"SCALPEL_"):
            print("[SCALPEL QUALITY FAIL]"); recorder._reset(); continue

        sp_err=torch.linalg.norm((sp_fp-scalpel_slot_w)[:2]).item()
        sp_ok2=sp_mz>sp_cz+0.06 and sp_err<0.09 and float(sp_fp[2])<0.12
        print(f"[SCALPEL RESULT] mz={sp_mz:.4f} err={sp_err:.4f} fz={float(sp_fp[2]):.4f} ok={sp_ok2}")

        if not sp_ok2:
            print("[PHASE FAIL] scalpel place"); recorder._reset(); continue
        ...
        success=sp_ok2
        meta={
            "task_mode":"scalpel_only",
            "target_object":"scalpel",
            "phase_id":0,
            "attempt":attempt,
            "scalpel_pose_mode":scalpel_pose_mode,
            "scissor_spawn":json.dumps(spawn_params["scissor"]),
        ...
        pick_out_dir = os.path.join(args_cli.out_dir, "pick_policy", "scalpel")
        place_out_dir = os.path.join(args_cli.out_dir, "place_policy", "scalpel")
        ...
        meta.update({
            ...
            "distractor_objects": "scissor,love_retractor",
            "distractor_note": "current implementation has one distractor because only scalpel and scissor assets are configured",
        })

        ok_pick = save_realcompat_segment(recorder, saved_count, "pick", pick_out_dir, success, "scalpel", meta=meta)
        ok_place = save_realcompat_segment(recorder, saved_count, "place", place_out_dir, success, "scalpel", meta=meta)
    ...
    compute_and_save_realcompat_norm_stats(os.path.join(args_cli.out_dir, "pick_policy", "scalpel"))
    compute_and_save_realcompat_norm_stats(os.path.join(args_cli.out_dir, "place_policy", "scalpel"))
    print("[DONE]")
