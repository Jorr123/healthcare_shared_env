...
ENV_REQUEST = RecorderEnvRequest(
    target="kelly",
    distractors=("scalpel", "scissor", "love_retractor", "scalpel_type2"),
    environment=None,
...
KELLY_GRIP_STAGE_EXTRA_DOWN_Z = 0.0140  # z-only lower for Kelly grip stages
KELLY_GRIP_STAGE_Z_MIN = 0.0010
...
# Kelly: keep old working grasp logic.
KELLY_GRASP_ABOVE_TABLE = 0.018
KELLY_BODY_OFFSET_X = 0.0
KELLY_BODY_OFFSET_Y = 0.0
KELLY_BODY_OFFSET_Z = 0.004
KELLY_SPAWN_ROOT_Z = 0.0140
# ScalpelType2 constants
...
KELLY_RANDOM_X_RANGE = (0.34, 0.64)
KELLY_RANDOM_Y_RANGE = (0.04, 0.32)
...
# if target=scalpel -> distractor=kelly
# if target=kelly -> distractor=scalpel
...
KELLY_SLOT_OFFSET = [0.0,  0.055, 0.025]
...
parser.add_argument("--task_text",     type=str,
    default="Pick up the surgical kellys and place them in the left tray slot")
...
STATE_KEYS = [
    ...
    "ee_pos_bx","ee_pos_by","ee_pos_bz",
    "ee_quat_bw","ee_quat_bx","ee_quat_by","ee_quat_bz",
    "kelly_pos_bx","kelly_pos_by","kelly_pos_bz",
    "kelly_quat_bw","kelly_quat_bx","kelly_quat_by","kelly_quat_bz",
...
def get_kelly_pos_w(env):  return env.scene["object"].data.root_pos_w[0].clone()
def get_kelly_quat_w(env): return env.scene["object"].data.root_quat_w[0].clone()
...
def make_state(env, active_target_w, phase_id):
    ...
    st = torch.cat([
        ...
        to_base_pos(env,  get_kelly_pos_w(env)),
        to_base_quat(env, get_kelly_quat_w(env)),
...
def step_dynamic(env, recorder, name, target_w, quat_w, grip,
    ...
    for i in range(max_steps):
        ...
        obj = get_scalpel_pos_w(env) if phase_id==0 else get_kelly_pos_w(env)
...
def step_hold_const_grip(env, recorder, name, target_w, quat_w,
    ...
    Use this for kelly close: no delayed ramp, so the fingers clamp before lift.
    """
    max_z = -999.0
    for i in range(steps):
        st = make_state(env, tray_slot_w, phase_id)
...
class EpisodeRecorder:
    ...
    def save_episode(self, ep_idx, success, meta=None):
        ...
        with h5py.File(path, "w") as f:
            ...
            f.attrs["pick_order"] = "kelly_only"
            f.attrs["task_mode"] = "kelly_only"
            f.attrs["scalpel_slot"] = "right"
            f.attrs["kelly_slot"] = "left"
...
def _sample_extra_obj_params(rng, obj_name):
    ...
    if obj_name == "kelly":
        # Kelly uses x/y root, same pick table region.
...
def ensure_two_distractors(spawn_params, target_object, rng):
    """Ensure spawn_params contains scissor, scalpel, kelly.
...
def sample_episode_spawn_grid(rng, attempt, target_object):
    ...
    for _ in range(SPAWN_MAX_TRIES):
        if target_object == "scalpel":
            ...
            sc = {
                "x": float(rng.uniform(*KELLY_RANDOM_X_RANGE)),
                "y": float(rng.uniform(*KELLY_RANDOM_Y_RANGE)),
...
def save_realcompat_segment(recorder, ep_idx, segment, out_dir, success, object_name, meta=None):
    ...
    with h5py.File(path, "w") as f:
        ...
        f.attrs["distractor_policy_note"] = "target=kelly; current configured distractor: scalpel; scissor requires adding third RigidObjectCfg"
        f.attrs["state_frame"] = "robot_base"
        f.attrs["action_frame"] = "robot_base"
        f.attrs["action_type"] = "absolute_pose_quat_gripper_8d_baseframe"
        phase3_hide_all_debug_visuals(omni.usd.get_context().get_stage())
        ...
        dbg.create_dataset("kelly_pose_b", data=states_legacy[:, 16:23])
        dbg.create_dataset("scalpel_pose_b", data=states_legacy[:, 23:30])
        if object_name_l == "kelly":
...
def sample_tray_center(rng, device, kelly_pos_w, scalpel_center_w):
...
def phase3_compute_kelly_world_long_yaw(stage):
    """
    Estimate Kelly's PHYSICAL long axis from the actual rendered USD meshes.

    This avoids assuming that Kelly's root local X/Y axis is the instrument
    shaft direction. All mesh vertices are transformed to world coordinates,
    then PCA in world XY gives the principal instrument axis.

    Returns:
        yaw tensor-compatible float in radians, or None on failure.
    """
    import numpy as np
    from pxr import UsdGeom

    # Current Phase3 uses one env, but search fallback is included.
    root = stage.GetPrimAtPath("/World/envs/env_0/Kelly")

    if not root or not root.IsValid():
        root = None
        for prim in stage.Traverse():
            if str(prim.GetPath()).endswith("/Kelly"):
                root = prim
                break

    if root is None or not root.IsValid():
        print("[KELLY PCA WARN] Kelly prim not found")
        return None
    ...
    try:
        for prim in Usd.PrimRange(root):
            ...
            # Sampling is enough for PCA and avoids unnecessary cost.
        ...
        print("[KELLY PCA WARN] mesh traversal failed:", repr(e))
        return None

    if len(pts_world) < 8:
        print(
            "[KELLY PCA WARN] insufficient mesh points:",
            len(pts_world)
        )
        return None
    ...
    # Only tabletop direction matters.
    XY = P[:, :2]
    XY = XY - XY.mean(axis=0, keepdims=True)

    cov = XY.T @ XY / max(1, XY.shape[0] - 1)

    eigvals, eigvecs = np.linalg.eigh(cov)

    # Largest eigenvalue = physical long axis.
    v = eigvecs[:, int(np.argmax(eigvals))]

    # Axis sign is irrelevant for a parallel-jaw gripper.
    yaw = float(np.arctan2(v[1], v[0]))

    print(
        f"[KELLY PCA] points={len(P)} "
        f"long_axis_xy=({v[0]:+.4f},{v[1]:+.4f}) "
        f"long_yaw_deg={np.degrees(yaw):.1f}"
    )
...
# KELLY branch is kept as before.
...
def pick_and_place_object(
    ...
    if obj_name == "SCALPEL":
        ...
        # KELLY: same working yaw-only logic as old script.
        ...
        print(f"\n[KELLY] origin=({float(obj_raw[0]):.4f},{float(obj_raw[1]):.4f},{float(obj_raw[2]):.4f})")
        print(f"  grasp=({float(obj[0]):.4f},{float(obj[1]):.4f},{float(obj[2]):.4f})")

        # KELLY geometry-aware grasp.
        #
        # Do NOT use root obj_yaw as the Kelly shaft direction. The mesh's
        # physical long axis can be rotated relative to the USD root frame.
        #
        # We estimate the actual long axis from current world-space mesh
        # geometry, then use the SAME calibrated gripper-vs-long-axis offset
        # already used by the working scalpel controller.
        obj_yaw = yaw_from_quat_wxyz(obj_quat)

        kelly_long_yaw_value = phase3_compute_kelly_world_long_yaw(stage)

        if kelly_long_yaw_value is None:
            # Safe fallback only if mesh PCA fails.
            kelly_long_yaw = obj_yaw
            print(
                "[KELLY PCA FALLBACK] using root yaw = "
                f"{math.degrees(float(obj_yaw)):.1f} deg"
            )
        else:
            kelly_long_yaw = torch.tensor(
                kelly_long_yaw_value,
                device=env.device,
                dtype=obj_yaw.dtype,
            )

        # SCALPEL_GRASP_YAW_OFFSET is already calibrated for this
        # Franka/base_grip_quat closing-axis convention.
        grasp_yaw = (
            kelly_long_yaw
            + torch.as_tensor(
                SCALPEL_GRASP_YAW_OFFSET,
                device=env.device,
                dtype=obj_yaw.dtype,
            )
        )

        # Parallel jaws are equivalent under pi rotation.
        grasp_yaw = torch.remainder(
        ...
        grasp_quat = quat_mul(
            yaw_q.reshape(1,4),
            base_grip_quat.reshape(1,4),
        )[0]

        print(
            f"  [KELLY GEOM GRASP] "
            f"root_yaw={math.degrees(float(obj_yaw)):.1f}deg "
            f"long_yaw={math.degrees(float(kelly_long_yaw)):.1f}deg "
            f"grasp_yaw={math.degrees(float(grasp_yaw)):.1f}deg "
            f"calib_offset={math.degrees(float(SCALPEL_GRASP_YAW_OFFSET)):.1f}deg"
        )
    ...
    kelly_lower_grasp = phase3_kelly_lower_pos_zonly(grasp, env.device)
    kelly_lower_close_grasp = phase3_kelly_lower_pos_zonly(close_grasp, env.device)
    print(f"[KELLY ZONLY LOWER] dz={KELLY_GRIP_STAGE_EXTRA_DOWN_Z:.4f} grasp_z={float(grasp[2]):.4f}->{float(kelly_lower_grasp[2]):.4f} close_z={float(close_grasp[2]):.4f}->{float(kelly_lower_close_grasp[2]):.4f}")
    ...
    mz=max(mz,step_dynamic(env,recorder,pfx+"LOWER_GRASP", kelly_lower_grasp,      grasp_quat,1.0, tray_slot_target_w,phase_id,max_steps=120,dist_thresh=0.008,settle_steps=8,force_wait=_force))

    if obj_name == "SCALPEL":
        mz=max(mz,step_dynamic(env,recorder,pfx+"LOWER_EXTRA", kelly_lower_close_grasp, grasp_quat,1.0, tray_slot_target_w,phase_id,max_steps=50,dist_thresh=0.006,settle_steps=8,force_wait=True))

    hold_steps=20 if obj_name=="KELLY" else 30
    mz=max(mz,step_smooth_grip(env,recorder,pfx+"HOLD_BEFORE_CLOSE",kelly_lower_close_grasp,grasp_quat,
    ...
    if obj_name == "KELLY":
        # KELLY: close must be immediate and stationary.
        # Do not use step_smooth_grip here, because its first 25% stays open
        # and the kelly can slip before the fingers fully clamp.
        mz=max(mz,step_hold_const_grip(env,recorder,pfx+"CLOSE",kelly_lower_close_grasp,grasp_quat,
                                        -1.0,tray_slot_target_w,phase_id,steps=100))

        mz=max(mz,step_hold_const_grip(env,recorder,pfx+"HOLD_AFTER_CLOSE",kelly_lower_close_grasp,grasp_quat,
                                        -1.0,tray_slot_target_w,phase_id,steps=70))

        obj_after_close=get_pos_fn(env); close_z=float(obj_after_close[2])

        kelly_micro_lift = close_grasp + torch.tensor([0.0,0.0,0.070], device=env.device)
        mz=max(mz,step_dynamic(env,recorder,pfx+"MICRO_LIFT",kelly_micro_lift,grasp_quat,-1.0,
        ...
        mz=max(mz,step_smooth_grip(env,recorder,pfx+"CLOSE",kelly_lower_close_grasp,grasp_quat,
    ...
    lift_thresh=0.006 if obj_name=="KELLY" else 0.008
...
PHASE3_TARGET_OBJECT = "kelly"
PHASE3_TARGET_OBJECT_TYPE_ID = 3
...
def force_kelly_pose(env, spawn_params):
    ...
    # Kelly is the canonical target read by get_kelly_pos_w().
    phase3_write_rigid_root_pose(env, "object", [x, y, z], q, "KELLY")
...
# =============================================================================
# KELLY Z-ONLY LOWER FOR GRIP STAGES
# =============================================================================

def phase3_kelly_lower_pos_zonly(pos, device):
    q = pos.clone()
    dz = float(globals().get("KELLY_GRIP_STAGE_EXTRA_DOWN_Z", 0.0060))
    zmin = float(globals().get("KELLY_GRIP_STAGE_Z_MIN", 0.0010))
    q[2] = torch.clamp(q[2] - dz, min=zmin)
    return q
...
def main():
    ...
    print("[CONFIG] KELLY ONLY | direct random spawn | same schema for shared multitask training")
    ...
    print(f"  KELLY_USD_PATH={KELLY_USD_PATH}")
    print(f"  KELLY_SCALE={KELLY_SCALE}")
    phase3_hide_all_debug_visuals(omni.usd.get_context().get_stage())
    ...
    print("  KELLY_CLOSE=hard constant -1.0, then HOLD_AFTER_CLOSE before micro-lift")
    ...
    env_cfg.scene.object.spawn=UsdFileCfg(
        usd_path=KELLY_USD_PATH,scale=KELLY_SCALE,
    ...
    while saved_count < target_success:
        ...
        spawn_params=sample_episode_spawn_grid(rng, attempt, "kelly")
        spawn_params=ensure_two_distractors(spawn_params, "kelly", rng)
        spawn_params=phase3_ensure_all_5_objects_in_spawn(spawn_params, "kelly", rng)
        ...
        force_extra_distractors(env, spawn_params, "kelly")
        ...
        kelly_raw=get_kelly_pos_w(env)
        scalpel_center=get_scalpel_center_w(env)

        dist_obj=torch.linalg.norm((kelly_raw-scalpel_center)[:2]).item()
        if dist_obj < SPAWN_MIN_OBJ_OBJ:
            print(f"[SPAWN FAIL] too close after pose write ({dist_obj:.3f}m)"); recorder._reset(); continue

        tray_center=sample_tray_center(rng, env.device, kelly_raw, scalpel_center)
        ...
        kelly_slot_w=tray_center+torch.tensor(KELLY_SLOT_OFFSET,device=env.device)
        scalpel_slot_w=tray_center+torch.tensor(SCALPEL_SLOT_OFFSET,device=env.device)
        print(f"  kelly slot: {[round(x,4) for x in kelly_slot_w.tolist()]}")
        print(f"  scalpel slot: {[round(x,4) for x in scalpel_slot_w.tolist()]}")

        # ?? SINGLE-OBJECT PHASE: KELLY ONLY ?????????????????????????????
        print("\n[PHASE] Kelly -> left slot | phase_id=1")
        sc_ok,sc_mz,sc_cz,sc_fp=pick_and_place_object(
            env,recorder,stage,"KELLY",3,
            KELLY_GRASP_ABOVE_TABLE,
            KELLY_BODY_OFFSET_X,KELLY_BODY_OFFSET_Y,KELLY_BODY_OFFSET_Z,
            get_kelly_pos_w,get_kelly_quat_w,kelly_slot_w,base_grip_quat)

        if not sc_ok:
            print("[PHASE FAIL] kelly micro-lift"); recorder._reset(); continue
        if not quality_ok(recorder,"KELLY_"):
            print("[KELLY QUALITY FAIL]"); recorder._reset(); continue

        sc_err=torch.linalg.norm((sc_fp-kelly_slot_w)[:2]).item()
        sc_ok2=sc_mz>sc_cz+0.06 and sc_err<0.09 and float(sc_fp[2])<0.12
        print(f"[KELLY RESULT] mz={sc_mz:.4f} err={sc_err:.4f} fz={float(sc_fp[2]):.4f} ok={sc_ok2}")

        if not sc_ok2:
            print("[PHASE FAIL] kelly place"); recorder._reset(); continue
        ...
        success=sc_ok2
        meta={
            "task_mode":"kelly_only",
            "target_object":"kelly",
            "phase_id":1,
            "attempt":attempt,
            "scalpel_pose_mode":scalpel_pose_mode,
            "kelly_spawn":json.dumps(spawn_params["kelly"]),
        ...
        pick_out_dir = os.path.join(args_cli.out_dir, "pick_policy", "kelly")
        place_out_dir = os.path.join(args_cli.out_dir, "place_policy", "kelly")
        ...
        ok_pick = save_realcompat_segment(recorder, saved_count, "pick", pick_out_dir, success, "kelly", meta=meta)
        ok_place = save_realcompat_segment(recorder, saved_count, "place", place_out_dir, success, "kelly", meta=meta)
    ...
    compute_and_save_realcompat_norm_stats(os.path.join(args_cli.out_dir, "pick_policy", "kelly"))
    compute_and_save_realcompat_norm_stats(os.path.join(args_cli.out_dir, "place_policy", "kelly"))
    print("[DONE]")
