...
ENV_REQUEST = RecorderEnvRequest(
    target="scissor",
    distractors=("scalpel", "love_retractor", "kelly", "scalpel_type2"),
    environment=None,
...
parser.add_argument("--task_text",     type=str,
    default="Pick up the surgical scissors and place them in the left tray slot")
...
class EpisodeRecorder:
    ...
    def save_episode(self, ep_idx, success, meta=None):
        ...
        with h5py.File(path, "w") as f:
            ...
            f.attrs["pick_order"] = "scissor_only"
            f.attrs["task_mode"] = "scissor_only"
...
def save_realcompat_segment(recorder, ep_idx, segment, out_dir, success, object_name, meta=None):
    ...
    with h5py.File(path, "w") as f:
        ...
        f.attrs["distractor_policy_note"] = "target=scissor; expected distractors: scalpel and optional love_retractor when configured"
        f.attrs["state_frame"] = "robot_base"
        f.attrs["action_frame"] = "robot_base"
        f.attrs["action_type"] = "absolute_pose_quat_gripper_8d_baseframe"
        phase3_hide_all_debug_visuals(omni.usd.get_context().get_stage())
...
PHASE3_TARGET_OBJECT = "scissor"
PHASE3_TARGET_OBJECT_TYPE_ID = 1
...
def main():
    ...
    print("[CONFIG] SCISSOR ONLY | direct random spawn | same schema for shared multitask training")
    ...
    env_cfg.scene.object.spawn=UsdFileCfg(
        usd_path=SCISSOR_USD,scale=SCISSOR_SCALE,
    ...
    if hasattr(env_cfg,"events") and hasattr(env_cfg.events,"reset_object_position"):
        env_cfg.events.reset_object_position.params["asset_cfg"].body_names=["Surgical_scissors_Metal_0"]
    ...
    while saved_count < target_success:
        ...
        spawn_params=sample_episode_spawn_grid(rng, attempt, "scissor")
        spawn_params=ensure_two_distractors(spawn_params, "scissor", rng)
        spawn_params=phase3_ensure_all_5_objects_in_spawn(spawn_params, "scissor", rng)
        ...
        force_extra_distractors(env, spawn_params, "scissor")
        ...
        # ?? SINGLE-OBJECT PHASE: SCISSOR ONLY ?????????????????????????????
        print("\n[PHASE] Scissor -> left slot | phase_id=1")
        sc_ok,sc_mz,sc_cz,sc_fp=pick_and_place_object(
            env,recorder,stage,"SCISSOR",1,
            SCISSOR_GRASP_ABOVE_TABLE,
            SCISSOR_BODY_OFFSET_X,SCISSOR_BODY_OFFSET_Y,SCISSOR_BODY_OFFSET_Z,
            get_scissor_pos_w,get_scissor_quat_w,scissor_slot_w,base_grip_quat)

        if not sc_ok:
            print("[PHASE FAIL] scissor micro-lift"); recorder._reset(); continue
        if not quality_ok(recorder,"SCISSOR_"):
            print("[SCISSOR QUALITY FAIL]"); recorder._reset(); continue

        sc_err=torch.linalg.norm((sc_fp-scissor_slot_w)[:2]).item()
        sc_ok2=sc_mz>sc_cz+0.06 and sc_err<0.09 and float(sc_fp[2])<0.12
        print(f"[SCISSOR RESULT] mz={sc_mz:.4f} err={sc_err:.4f} fz={float(sc_fp[2]):.4f} ok={sc_ok2}")

        if not sc_ok2:
            print("[PHASE FAIL] scissor place"); recorder._reset(); continue
        ...
        success=sc_ok2
        meta={
            "task_mode":"scissor_only",
            "target_object":"scissor",
            "phase_id":1,
            "attempt":attempt,
        ...
        pick_out_dir = os.path.join(args_cli.out_dir, "pick_policy", "scissor")
        place_out_dir = os.path.join(args_cli.out_dir, "place_policy", "scissor")
        ...
        meta.update({
            ...
            "distractor_objects": "scalpel,love_retractor",
            "distractor_note": "current implementation has one distractor because only scalpel and scissor assets are configured",
        })

        ok_pick = save_realcompat_segment(recorder, saved_count, "pick", pick_out_dir, success, "scissor", meta=meta)
        ok_place = save_realcompat_segment(recorder, saved_count, "place", place_out_dir, success, "scissor", meta=meta)
    ...
    compute_and_save_realcompat_norm_stats(os.path.join(args_cli.out_dir, "pick_policy", "scissor"))
    compute_and_save_realcompat_norm_stats(os.path.join(args_cli.out_dir, "place_policy", "scissor"))
    print("[DONE]")
